...
假定时间帧索引$ 0 \leq t < T $, 偏移值索引$ 0 \leq i < I $.
那么通过最小化代价函数来实现pitch的追踪, 代价函数定义在一系列索引
$ \bold{s} = \left( s_t \right)_{t=0}^{T-1} $上,
对于任一时刻$ t $, 元素$ s_t $可以看作偏移值索引, 因此$ 0 \leq s_t < I $.

代价函数定义如下, 包含局部代价和惩罚项两个部分:

$$ C(\bold(s)) = \sum_{t=0}^{T-1} cost_{local} (t, s_t)
+ \sum_{t=1}^{T-1} penalty \cdot
\left( log \left(L_{s_t}/L_{s_{t-1}} \right) \right)^2 $$

其中, 局部代价为:

$$ cost_{local} (t, i) = 1 - \Phi_{t,i}(1 - f_{0, soft, min} L_i) $$

可以把$ 1 - \Phi_{t,i} $看作基础代价, 最小化代价即寻找NCCF峰值,
$ f_{0, soft, min} L_i $可以看作对较高偏移值的惩罚.

为了寻找最优序列$ \bold(s) $, 使得代价函数最小化, 采用Viterbi算法.

定义$ b(t, i) $为第$ t $帧第$ i $个偏移值处的Viterbi回溯,
其中包含了第$ t - 1 $帧最优的偏移值索引, 从而可以从$ b(t, i) $回溯最优路径.

基础的Viterbi算法的时间复杂度为$ O(I^2) $. 由于转移代价的凸函数特性, 可得:

$$ b(t, i + 1) \geq b(t, i) $$

因此, 可以优化Viterbi算法的复杂度近似$ O(I) $.

定义前向代价为$ c(t, i) $.
对于时刻$ t $, 偏移值从$ i = 0 $ 到$ i = I - 1 $遍历进行前向计算,
在计算$ c(t, i) $和$ b(t, i) $时,
仅使用$ c(t - 1, j), \ b(t, i - 1) \leq j \leq i $.

之后, 在从$ i = I - 1 $到$ i = 0 $进行后向计算时,
对前向代价$ c(t-1, j), \ i + 1 \leq j \leq b(t, i + 1) $进行遍历,
看是否能够找到比更好的前向代价和回溯.

完成前向后向后, 得到最优路径$ \bold(s) = \left( s_t \right)_{t=0}^{T-1} $.
那么, 可以得到每一帧的pitch为$ 1/L_{s_t} $, 每一帧的nccf为$ \Phi_{t, s_t} $.
